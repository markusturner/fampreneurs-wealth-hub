...
-- Update create_post_notification to scope by program/community group
...
BEGIN
  ...
  -- Map program slug to community group name
  ...
  -- Only notify members of the matching community group
  IF target_group_name IS NOT NULL THEN
    ...
    END LOOP;
  ELSE
    -- Fallback: no program set, notify all users
    FOR member_record IN 
      SELECT DISTINCT user_id FROM public.profiles WHERE user_id != NEW.user_id
    LOOP
      INSERT INTO public.notifications (
        user_id, sender_id, notification_type, title, message, reference_id
      ) VALUES (
...
-- Update notify_community_post to scope by program/community group
