...
// Submission limits: tools are unlimited, trust forms = 1
const getSubmissionLimit = (type: SectionType): number => {
  if (TOOL_TYPES.includes(type) || type === 'family_protection_plan') return Infinity
  return 1
}
      ...
      <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-base font-semibold text-foreground">{trustAccess?.program === 'tfv' ? "Schedule B & Proof of Transfer" : "Schedule B, Proof of Transfer & Complimentary Calls"}</h2>
          </div>
          <div className={`grid gap-3 grid-cols-1 sm:grid-cols-3`}>
            {(['trust_asset_uploads'] as SectionType[]).map(type => renderSectionCard(type))}
            {trustAccess?.program !== 'tfv' && [
              {
                title: "Call w/ Attorney Domonique Price",
                url: "https://calendly.com/dprice-2/estateplanning-withprice?month=2023-08",
              },
              {
                title: "Call w/ Insurance Agent Ari Davis",
                url: "https://calendly.com/arzeriadavis",
              },
            ].map((item) => (
              <Card
                key={item.url}
